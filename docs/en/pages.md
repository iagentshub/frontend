...
| **Knowledge** | Knowledge management for attaching context to agents. Organized in three tabs: **Skills** (browse available skills with a Load button to import a private JSON), **URLs** (save web pages whose text is extracted when added), and **Documents** (upload `.txt`, `.md`, or `.pdf` files with drag & drop support). Each item's content is injected in full into the agent's system prompt at chat start. |
