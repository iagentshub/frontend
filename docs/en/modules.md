...
| Dashboard | `dashboard/dashboard.js` (orchestrator: 4-column layout, edit mode, drag-and-drop, config card flip, persistence) + one module per widget in `dashboard/widgets/{id}/widget.js`: `summary`, `token-usage`, `activity`, `conn-status`, `recent`. Widgets self-register in `window._WIDGET_REGISTRY` and are loaded via `<script>` tags before the orchestrator. Each widget receives a `cols` property (1/2/4) and renders different content depending on the size. The `conn-status` and `recent` widgets handle client-side pagination internally (`pageSize`, `.w-pager` controls). The config form (`.wcfg-sizes` for size, `.wcfg-pills` for remaining options) is defined per widget and shown when the card is flipped. |
