...
| `assets/components/workspace-share-dialog/workspace-share-dialog.js` | Modal para compartir un recurso con TODO un workspace (sin grupos, sin mover ni copiar el recurso); se invoca con `WorkspaceShareDialog.open(type, id, name)` |
| `assets/components/workspace-create-wizard/workspace-create-wizard.js` | Asistente de creación de workspace (nombre → ¿añadir contenido? → enlazar agentes/skills/conocimiento o compartir conexiones); se invoca con `WorkspaceCreateWizard.open()` |
...
| Dashboard | `dashboard/dashboard.js` (orquestador: layout de 4 columnas, modo edición, drag-and-drop, flip de config, persistencia) + un módulo por widget en `dashboard/widgets/{id}/widget.js`: `summary`, `token-usage`, `activity`, `conn-status`, `recent`. Los widgets se registran en `window._WIDGET_REGISTRY` y se cargan con `<script>` antes que el orquestador. Cada widget recibe la propiedad `cols` (1/2/4) y renderiza contenido diferente según el tamaño. Los widgets `conn-status` y `recent` gestionan paginación cliente-side internamente (`pageSize`, controles `.w-pager`). El formulario de configuración (`.wcfg-sizes` para tamaño, `.wcfg-pills` para el resto de opciones) se define en cada widget y se muestra al voltear la card. |
...
| Perfil | `profile/profile.js`, `profile/profile-workspaces.js` (gestión de miembros e invitaciones del workspace), `profile/profile-accounts.js` |
