...
| **Dashboard** | Vista general personalizable de la plataforma. El grid usa 4 columnas; cada panel tiene tres tamaños seleccionables: **Pequeño** (1 columna, 25%), **Mediano** (2 columnas, 50%) y **Grande** (4 columnas, 100%). El tamaño no solo cambia el ancho — también adapta el contenido renderizado: el **Resumen** muestra lista compacta / cards sin icono / cards completas con icono y enlace; el **Uso de tokens** muestra solo el total / top 3 / top N con leyenda enriquecida; la **Actividad** muestra número hero + minibarras / histograma de 14 días / histograma completo configurable; el **Estado de conexiones** muestra contador hero / lista compacta / grid de tarjetas paginado; los **Agentes recientes** muestran lista de nombres / grid de 2 columnas / grid completo paginado. Los widgets **Estado de conexiones** y **Agentes recientes** incluyen paginación cliente-side con controles ← X/N → en el footer. El botón **Personalizar** activa el modo edición: los paneles se pueden reordenar arrastrando, añadir desde el panel lateral y eliminar con el botón ×. Cada panel tiene un icono de engranaje que voltea la card para mostrar sus opciones de configuración; el selector de tamaño es visual (barra proporcional 25%/50%/100%) y el resto de opciones usa pills tipo chip. El layout y la configuración de cada panel se persisten por usuario. Accesible en `/dashboard/`. |
