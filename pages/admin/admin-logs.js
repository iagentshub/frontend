...
(function () {
    ...
    function _fmtDay(raw) { return raw.slice(6,8)+'/'+raw.slice(4,6)+'/'+raw.slice(0,4); }
    function _today() {
        var d = new Date();
        ...
        if (!visible.length) {
            ...
            var rows = visible.map(function (l) {
                ...
                return '<tr>' +
                    ...
                    '<td class="logs-td logs-td-msg">' + _esc(l.message) + '</td>' +
                    '</tr>';
            });
    ...
    function _setLoading() {
        ...
        if (tbody) tbody.innerHTML =
            '<tr><td colspan="5" class="logs-td logs-td-empty">…</td></tr>';
    }
    ...
    function loadLog(date) {
        ...
        api.getText('/api/admin/logs/' + date).then(function (text) {
            _rawText = text || '';
            _lines   = _rawText.split('\n').filter(Boolean).map(_parseLine);
            _render();
            ...
            if (dl) {
                dl.style.display = '';
                dl.href     = 'data:text/plain;charset=utf-8,' + encodeURIComponent(_rawText);
                dl.download = date + '.log';
            }
        }).catch(function (e) {
            var tbody = document.getElementById('logs-tbody');
            if (tbody) tbody.innerHTML =
                '<tr><td colspan="5" class="logs-td logs-td-empty">' +
    ...
    function loadList() {
        api.get('/api/admin/logs').then(function (dates) {
            var sel = document.getElementById('logs-date-select');
            if (!sel) return;
            sel.innerHTML = '';
            ...
            var target = dates.indexOf(_today()) !== -1 ? _today() : dates[0];
            ...
            items.forEach(function (item) {
                ...
                card.innerHTML =
                    '<div class="lsc-header">' +
                        ...
                        '<span class="lsc-size">' + _fmtSize(item.size_bytes) + '</span>' +
                    '</div>' +
                    '<div class="lsc-lines">' + item.lines + ' líneas</div>' +
