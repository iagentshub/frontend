...
function renderGrid(memories, root) {
    ...
    if (!memories.length) {
        root.innerHTML = '<div class="mem-empty"><div class="mem-empty-icon">🗂️</div>' + esc(t('memory.empty')) + '</div>';
        return;
    }
...
function renderCard(m) {
    ...
    return '<article class="mem-card" data-file="' + esc(m.filename) + '">' +
        ...
        '<button class="mem-action mem-action--edit" data-action="edit" data-file="' + esc(m.filename) + '">' + esc(t('memory.actions.edit')) + '</button>' +
        '<button class="mem-action mem-action--delete" data-action="delete" data-file="' + esc(m.filename) + '">' + esc(t('memory.actions.delete')) + '</button>' +
        '</footer>' +
        '</article>';
}
